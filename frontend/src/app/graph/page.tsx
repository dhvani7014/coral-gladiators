...
const NODE_COLORS: Record<
  string,
  { bg: string; border: string; label: string; style: string }
> = {
  Entity: {
    bg: "#D97B4F",
    border: "#8B5E3C",
    label: "Entity",
    style: "bg-[#F5EAE0] text-[#D97B4F] border border-[#E8CDB8]",
  },
  Transaction: {
    bg: "#A87820",
    border: "#7A5810",
    label: "Transaction",
    style: "bg-[#F5EDD8] text-[#A87820] border border-[#E8D5A0]",
  },
  SanctionList: {
    bg: "#C0392B",
    border: "#8B1A1A",
    label: "Sanction",
    style: "bg-[#F5E8E8] text-[#C0392B] border border-[#E8C8C8]",
  },
  Email: {
    bg: "#3B72B8",
    border: "#1A4A8A",
    label: "Email",
    style: "bg-[#E3EDF8] text-[#3B72B8] border border-[#C8DCF0]",
  },
  SlackMessage: {
    bg: "#7B58B8",
    border: "#5A3A8A",
    label: "Slack",
    style: "bg-[#EDE8F5] text-[#7B58B8] border border-[#D5C8EC]",
  },
};
...
const PRESET_QUERIES = [
  ...
  },
  {
    ...
    code: "02",
    cypher:
      "MATCH (e:Entity)-[:INITIATED]->(t:Transaction)-[:SENT_TO]->(v:Entity) RETURN e,t,v LIMIT 50",
  },
  {
    label: "Sanctioned",
    code: "03",
    cypher:
      "MATCH (e:Entity)-[:INITIATED]->(t:Transaction)-[:INVOLVES_SANCTIONED]->(v:Entity) RETURN e,t,v",
  },
  {
    label: "Emails",
    code: "04",
    cypher: "MATCH (e:Entity)-[:SENT_EMAIL]->(m:Email) RETURN e,m LIMIT 30",
  },
  {
    label: "Slack",
    code: "05",
    cypher: "MATCH (e:Entity)-[:POSTED]->(s:SlackMessage) RETURN e,s",
  },
];
  ...
  for (const result of data.results ?? []) {
    for (const row of result.data ?? []) {
      for (const cell of row.graph?.nodes ?? []) {
        ...
        const colors = NODE_COLORS[label] ?? {
          bg: "#9B8E82",
          border: "#6B5E52",
        };
        const caption =
...
function loadVis(): Promise<any> {
  return new Promise((resolve, reject) => {
    ...
    script.onload = () =>
      (window as any).vis
        ? resolve((window as any).vis)
        : reject(new Error("vis not on window"));
...
export default function GraphPage() {
  ...
  return (
    <>
      ...
      <div
        className="font-outfit bg-[#F7F3EE] text-[#1A1612] flex flex-col"
        style={{ height: "100vh" }}
      >
        {/* ── Top bar ── */}
          ...
          <div className="flex items-center gap-5">
            ...
            {status === "ready" && (
              <div className="flex items-center gap-2 bg-[#2A2420] border border-[#3A3430] rounded-full px-3 py-[5px]">
                ...
                <span className="text-[11px] text-[#3B8A52] tracking-wider font-medium">
                  Connected
                </span>
              </div>
            )}
            {status === "error" && (
              <div className="flex items-center gap-2 bg-[#2A2420] border border-[#3A3430] rounded-full px-3 py-[5px]">
                ...
                <span className="text-[11px] text-[#C0392B] tracking-wider font-medium">
                  Error
                </span>
              </div>
            )}
              ...
              {PRESET_QUERIES.map((pq, i) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    <span className="preset-num text-[10px] font-medium tracking-[0.12em] text-[#C4B8AC]">
                      {pq.code}
                    </span>
                    <span className="text-[12px] font-medium text-[#4A3E35]">
                      {pq.label}
                    </span>
                  </div>
                  <span className="preset-arrow text-[11px] text-[#D97B4F]">
                    →
                  </span>
                </button>
              ))}
              ...
              <div className="px-5 pb-4 flex flex-col gap-2.5">
                {Object.entries(NODE_COLORS).map(([label, c]) => (
                  <div key={label} className="flex items-center gap-2.5">
                    ...
                    />
                    <span
                      className={`text-[10px] font-medium px-2 py-0.5 rounded-full border ${c.style}`}
                    >
                      {c.label}
                    </span>
                  </div>
              ...
              <div className="flex-1">
                ...
                <div className="px-5 py-4 flex flex-col gap-3">
                  {Object.entries(selected).map(([k, v]) => (
                    <div key={k}>
                      <p className="text-[10px] tracking-[0.12em] text-[#C4B8AC] uppercase mb-0.5">
                        {k}
                      </p>
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
            ...
            <div className="flex-1 relative overflow-hidden bg-[#FDFAF7]">
              ...
              <div
                ...
                style={{
                  backgroundImage:
                    "radial-gradient(circle, #D8CEBF 1px, transparent 1px)",
              ...
              {status === "error" && (
                <div className="absolute inset-0 flex items-center justify-center z-10">
                  <div className="text-center bg-white border border-[#E0D8CF] rounded-xl p-10 shadow-sm max-w-sm mx-auto">
                    <div className="font-playfair text-5xl text-[#E0D8CF] mb-4">
                      ⚠
                    </div>
                    <p className="font-playfair text-xl font-bold text-[#C0392B] mb-2">
                      Graph Error
                    </p>
                    <p className="text-[12px] font-light text-[#9B8E82] break-all mb-6 leading-relaxed">
                      {errorMsg}
                    </p>
                    <button
                      onClick={() => buildGraph(query)}
