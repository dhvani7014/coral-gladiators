...
type Stage = { id: string; label: string; status: StageStatus; detail?: string };
...
function stageStyles(status: StageStatus) {
    switch (status) {
        case "done": return { card: "border-[#C0DCC8] bg-[#E3F0E8]/40", dot: "bg-[#3B8A52]", text: "text-[#3B8A52]" };
        case "running": return { card: "border-[#D97B4F] bg-[#F5EAE0]/40", dot: "bg-[#D97B4F]", text: "text-[#D97B4F]" };
        case "error": return { card: "border-[#E8CDB8] bg-[#F5EAE0]/20", dot: "bg-[#C06030]", text: "text-[#C06030]" };
        default: return { card: "border-[#E0D8CF] bg-[#F7F3EE]", dot: "bg-[#D8CEBF]", text: "text-[#C4B8AC]" };
    }
}

const detailFromDone = (agentName: string, data: any): string => {
    switch (agentName) {
        case "Planner": return `Target: ${data.target} · ${data.query_count} queries`;
        case "SQL": return `${data.total_rows} rows across ${data.queries?.length ?? 0} queries`;
        case "Graph": return `${Object.values(data.nodes ?? {}).reduce((a: any, b: any) => a + b, 0)} nodes`;
        case "Fraud": return `Score: ${data.rule_score}/100 · ${data.risk_level}`;
        case "GraphIntelligence": return data.findings?.[0] ?? `${data.network_size} connected nodes`;
        case "Report": return `${data.risk_level} · ${data.recommended_action}`;
        ...
        try {
            ...
            while (true) {
                ...
                for (const part of parts) {
                    ...
                    if (eventType === "agent_start") setStage(stageId, "running", data.message);
                    if (eventType === "agent_done") setStage(stageId, "done", detailFromDone(data.agent, data));
                    if (eventType === "agent_error") setStage(stageId, "error", data.error);
            ...
            <div className="font-outfit bg-[#F7F3EE] min-h-screen text-[#1A1612]">
                ...
                <div className="bg-[#1A1612] flex items-center justify-between px-12 h-[52px]">
                    <div className="flex items-center gap-6">
                        ...
                        <span className="text-[11px] text-[#6B5E52] tracking-wide">Investigate</span>
                    </div>
                    <div className="flex items-center gap-4">
                        {loading && (
                            <div className="flex items-center gap-2">
                                <div className="w-1.5 h-1.5 rounded-full bg-[#D97B4F] animate-pulse-dot" />
                                <span className="text-[11px] text-[#D97B4F] tracking-wide">Pipeline running</span>
                            </div>
                        )}
                        <span className="text-[11px] text-[#3A3430] tracking-wide">6 agents</span>
                    </div>
                </div>
                ...
                <div className="px-12 pt-12 pb-10 border-b border-[#E0D8CF]">
                    ...
                    <h1 className="font-playfair text-[52px] font-black leading-[0.92] tracking-tight text-[#1A1612] mb-8">
                        Investigate <em className="italic text-[#8B5E3C]">any entity.</em>
                    </h1>
                ...
                <div className="px-12 py-8">
                    ...
                    <div className="mb-px">
                        <div className="flex items-center justify-between mb-4">
                            <span className="text-[11px] font-medium tracking-[0.2em] text-[#9B8E82] uppercase">Pipeline</span>
                                ...
                                return (
                                    ...
                                    >
                                        <div className="flex items-center justify-between mb-3">
                                            ...
                                            <div className={`w-2 h-2 rounded-full flex-shrink-0 ${st.dot} ${stage.status === "running" ? "animate-pulse-dot" : ""}`} />
                                        </div>
                                        <p className="text-[12px] font-medium text-[#1A1612] mb-1 leading-tight">{stage.label}</p>
                                        {stage.detail
                                            ? <p className="text-[11px] font-light text-[#9B8E82] truncate leading-snug">{stage.detail}</p>
                                            : <p className={`text-[11px] tracking-wide ${st.text}`}>
                                                {stage.status === "idle" ? "Waiting" : stage.status === "running" ? "Running…" : stage.status === "done" ? "Done" : "Error"}
                                            </p>
                                        }
                                    </div>
                                );
                        ...
                        <div className="mt-8 space-y-px">
                                    ...
                                    <div>
                                        <div className="flex items-center gap-2.5 mb-4">
                                            ...
                                            <span className="text-[11px] font-medium tracking-[0.18em] text-[#9B8E82] uppercase">
                                                Fraud risk — {result.target}
                                            </span>
                                        </div>
                                        <div className="flex items-baseline gap-3 mb-2">
                                            <span className={`font-playfair text-[72px] font-black leading-none ${risk.score}`}>
                                                {result.risk_score}
                                            </span>
                                            <span className="text-[#C4B8AC] text-2xl font-light">/100</span>
                                        </div>
                                        <span className={`text-[11px] font-semibold tracking-[0.15em] uppercase px-3 py-1 rounded-full ${risk.pill}`}>
                                            {result.risk_level}
                                        </span>
                                    </div>
                                    <div className={`text-right mt-1`}>
                                        <p className="text-[10px] text-[#9B8E82] tracking-widest uppercase mb-2">Recommendation</p>
                                        <p className="text-[13px] font-medium text-[#1A1612] max-w-[240px] leading-snug">{result.recommendation}</p>
                                    </div>
                                </div>
                                <p className="text-[14px] font-light text-[#6B5E52] leading-relaxed mt-6 max-w-3xl">{result.summary}</p>
                            </div>

                            {/* Key findings */}
                            <div className="bg-[#F7F3EE] border border-[#E0D8CF] p-8">
                                <p className="text-[11px] font-medium tracking-[0.2em] text-[#9B8E82] uppercase mb-5">Key findings</p>
                                <ul className="space-y-3">
                                    {(report.key_findings ?? assessment.key_findings ?? []).map((f: string, i: number) => (
                                        <li key={i} className="flex gap-4 text-[13px] text-[#6B5E52] font-light leading-relaxed">
                                            <span className="text-[#D97B4F] mt-0.5 flex-shrink-0">→</span>
                                            <span>{f}</span>
                                        </li>
                                    ))}
                                </ul>
                            ...
                            <div className="grid grid-cols-2 border border-[#E0D8CF]">
                                <div className="p-8 border-r border-[#E0D8CF] bg-[#F7F3EE]">
                                    <p className="text-[11px] font-medium tracking-[0.2em] text-[#9B8E82] uppercase mb-5">SQL evidence</p>
                                    <div className="space-y-3">
                                        {sqlResults.map((q: any) => (
                                            <div key={q.id} className="flex justify-between items-center">
                                                <span className="text-[13px] font-light text-[#6B5E52]">{q.name}</span>
                                                <span className={`text-[13px] font-medium ${q.count > 0 ? "text-[#1A1612]" : "text-[#C4B8AC]"}`}>
                                                    {q.count} rows
                                                </span>
                                            </div>
                                        ))}
                                ...
                                <div className="p-8 bg-[#F7F3EE]">
                                    <p className="text-[11px] font-medium tracking-[0.2em] text-[#9B8E82] uppercase mb-5">Graph nodes</p>
                                    <div className="space-y-3">
                                        {Object.entries(graphNodes).map(([type, count]: any) => (
                                            <div key={type} className="flex justify-between items-center">
                                                <span className="text-[13px] font-light text-[#6B5E52]">{type}</span>
                                                <span className="text-[13px] font-medium text-[#1A1612]">{count}</span>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                            ...
                            {graphIntel.findings?.length > 0 && (
                                <div className="bg-[#F7F3EE] border border-[#E0D8CF] p-8">
                                    <p className="text-[11px] font-medium tracking-[0.2em] text-[#9B8E82] uppercase mb-5">Graph intelligence</p>
                                    <div className="flex flex-wrap gap-2">
                                        {graphIntel.findings.map((f: string, i: number) => (
                                            <span key={i} className="text-[12px] font-light bg-[#E3F0E8] text-[#3B8A52] border border-[#C0DCC8] rounded-full px-3 py-1">
                                                {f}
                                            </span>
                                        ))}
                                    </div>
                            ...
                            {report.evidence?.length > 0 && (
                                <div className="bg-[#F7F3EE] border border-[#E0D8CF] p-8">
                                    <p className="text-[11px] font-medium tracking-[0.2em] text-[#9B8E82] uppercase mb-5">Evidence</p>
                                    <div className="divide-y divide-[#E0D8CF]">
                                        {report.evidence.map((e: any, i: number) => (
                                            <div key={i} className="flex gap-6 py-3">
                                                <span className="text-[12px] font-medium text-[#D97B4F] w-24 flex-shrink-0 pt-0.5">{e.source}</span>
                                                <span className="text-[13px] font-light text-[#6B5E52] leading-relaxed">{e.detail}</span>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                            ...
                            {/* Rule engine */}
                            <div className="bg-[#F7F3EE] border border-[#E0D8CF] p-8">
                                <p className="text-[11px] font-medium tracking-[0.2em] text-[#9B8E82] uppercase mb-5">Rule engine output</p>
                                <div className="space-y-2">
                                    {result.fraud_assessment?.rule_findings?.map((r: string, i: number) => (
                                        <div key={i} className="text-[12px] font-light text-[#6B5E52] bg-[#EDE7DF] border border-[#D8CEBF] rounded-md px-4 py-2.5">
                                            {r}
                                        </div>
                                    ))}
                                </div>
                            ...
                            {report.investigator_notes && (
                                <div className="bg-[#F5EDD8] border border-[#E8D5A0] rounded-b-lg p-8">
                                    <p className="text-[11px] font-medium tracking-[0.2em] text-[#A87820] uppercase mb-3">Investigator notes</p>
                                    <p className="text-[13px] font-light text-[#6B5E52] leading-relaxed">{report.investigator_notes}</p>
                                </div>
                            )}
                    ...
                    <div className="border-t border-[#E0D8CF] mt-8 pt-5 flex items-center justify-between">
                        <span className="text-[11px] text-[#C4B8AC] tracking-wide">Sentinel AI — Fraud investigation</span>
                        <div className="flex gap-6">
                            {[["Feed", "/feed"], ["Report", "/report"], ["Graph", "/graph"], ["← Home", "/"]].map(([label, href]) => (
                                <Link key={href} href={href} className="text-[11px] text-[#9B8E82] hover:text-[#1A1612] tracking-wide transition-colors no-underline">
                                    {label}
                                </Link>
                            ))}
                        </div>
