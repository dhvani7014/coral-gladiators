...
interface InvestigationResult {
  ...
  fraud_assessment?: {
    graph_evidence?: {
      sanctions: { hit_count: number; types: string[] };
      emails: { count: number; subjects: string[] };
      slack: { count: number; messages: string[] };
...
function downloadPDF(result: InvestigationResult, timestamp: string) {
  ...
  const ge = result?.fraud_assessment?.graph_evidence;
  ...
  function addText(
    ...
    doc.text(lines as string[], x, startY);
  ...
  doc.setFont("helvetica", "bold");
  doc.setTextColor(247, 243, 238);
  doc.text(report.title || `Investigation: ${result.target}`, margin, 36, {
    maxWidth: contentW - 35,
  });
  ...
  doc.setTextColor(107, 94, 82);
  const ts = timestamp
    ? new Date(timestamp).toLocaleString().toUpperCase()
    : new Date().toLocaleString().toUpperCase();
  doc.text(
    `GENERATED: ${ts}   |   TARGET: ${result.target.toUpperCase()}`,
    margin,
    44,
  );
  ...
  y = 58;
  doc.setFillColor(
    ...(riskColor.map((c) => Math.floor(c * 0.15)) as [number, number, number]),
  );
  ...
  if (ge) {
    ...
    const stats = [
      ["SANCTIONS HITS", String(ge.sanctions?.hit_count ?? 0)],
      ["EMAILS FOUND", String(ge.emails?.count ?? 0)],
      ["SLACK MENTIONS", String(ge.slack?.count ?? 0)],
      ["CORROBORATIONS", String(ge.corroborations ?? 0)],
    ...
    stats.forEach(([label, value], i) => {
      ...
      doc.setFont("helvetica", "bold");
      doc.setTextColor(...riskColor);
      doc.text(String(value), cx + cellW / 2 - 1, y + 8, { align: "center" });
      doc.setFontSize(6.5);
      doc.setFont("helvetica", "normal");
      doc.setTextColor(155, 142, 130);
      doc.text(String(label), cx + cellW / 2 - 1, y + 13, { align: "center" });
    });
    y += 20;
  }
  ...
  (report.key_findings || []).forEach((finding, i) => {
    ...
    doc.setFillColor(
      ...(riskColor.map((c) => Math.floor(c * 0.18)) as [
        number,
        number,
        number,
      ]),
  ...
  if (ge?.sanctions?.types && ge.sanctions.types.length > 0) {
    y = sectionHeader("SANCTIONS PROGRAMS", y);
    const types = [...new Set(ge.sanctions.types)];
    let fx = margin;
    types.forEach((t) => {
      const fw = doc.getTextWidth(t) + 8;
      if (fx + fw > W - margin) {
        fx = margin;
        y += 9;
      }
      ...
      doc.setFont("helvetica", "bold");
      doc.setTextColor(...riskColor);
      doc.text(t, fx + fw / 2, y + 5, { align: "center" });
      fx += fw + 3;
    });
    y += 13;
  }
  ...
  if (report.evidence && report.evidence.length > 0) {
    ...
    report.evidence.forEach((ev, i) => {
      ...
      doc.text(lines as string[], margin + 38, y + 6);
      y += rowH;
    });
  }
  ...
  for (let p = 1; p <= pageCount; p++) {
    ...
    doc.setTextColor(107, 94, 82);
    doc.text(
      "SENTINEL — CONFIDENTIAL — AUTOMATED FRAUD INVESTIGATION",
      margin,
      291,
    );
  ...
  doc.save(
    `sentinel_${result.target.replace(/\s+/g, "_").toLowerCase()}_${Date.now()}.pdf`,
  );
}
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      <svg width={220} height={160} viewBox="0 0 220 160">
        ...
        />
        <path
          d={arcPath(arcStart, arcStart + filled)}
          fill="none"
          stroke={color}
          strokeWidth={strokeW}
          strokeLinecap="round"
        ...
        <text
          x={polar(arcStart, r + 20).x}
          y={polar(arcStart, r + 20).y + 4}
          textAnchor="middle"
          fill="#C4B8AC"
        ...
        <text
          x={polar(arcEnd, r + 20).x}
          y={polar(arcEnd, r + 20).y + 4}
          textAnchor="middle"
          fill="#C4B8AC"
          style={{ fontSize: 9, fontFamily: "'Outfit', sans-serif" }}
        >
      ...
      <span
        className={`px-5 py-1.5 rounded-full text-xs font-semibold tracking-[0.18em] border font-outfit ${rc.text} ${rc.border} ${rc.bg}`}
      >
        {rc.label} RISK
      </span>
    </div>
...
function StatCard({
  ...
  return (
    <div className="bg-[#EDE7DF] border border-[#D8CEBF] rounded-md px-5 py-4 flex flex-col gap-1">
      ...
      <span
        className={`font-playfair text-2xl font-bold ${accent ? "text-[#D97B4F]" : "text-[#1A1612]"}`}
      >
        {value}
      </span>
    </div>
          ...
          <div className="flex items-end justify-between gap-10">
            <div>
              ...
              <p className="text-[14px] font-light text-[#6B5E52] leading-[1.75] max-w-[380px]">
                Risk gauge, key findings, evidence trail, and full agent output
            ...
            <div className="flex gap-3 flex-1 max-w-lg">
              <input
                ...
                placeholder='Enter a company or person name — e.g. "Zenith LLC"'
        ...
        <div className="px-12 py-10 pb-28">
          ...
          {loading && (
            <div className="text-center py-24">
              <div className="text-[11px] tracking-[0.2em] text-[#9B8E82] mb-8 uppercase">
                Running agent pipeline
              </div>
              <div className="flex justify-center gap-2">
                {["PLANNER", "SQL", "GRAPH", "FRAUD", "INTEL", "REPORT"].map(
                  (s, i) => (
                    <div
                      key={s}
                      className="px-3 py-1.5 border border-[#D8CEBF] rounded-full text-[10px] tracking-widest text-[#C4B8AC] font-outfit"
                      style={{
                        animation: `agent-pulse 1.5s ease-in-out ${i * 0.25}s infinite`,
                      }}
                    >
                      {s}
                    </div>
            ...
            <div className="flex flex-col gap-0 border border-[#E0D8CF] rounded-xl overflow-hidden">
              ...
              <div className="grid grid-cols-[1fr_auto] gap-10 items-center p-10 bg-white border-b border-[#E0D8CF]">
                <div>
                  ...
                  <span
                    className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-xs font-semibold tracking-[0.15em] border font-outfit ${rc.text} ${rc.border} ${rc.bg}`}
                  >
                ...
                {[
                  {
                    ...
                    value: ge?.sanctions?.hit_count ?? 0,
                    accent: true,
                  },
                  {
                    ...
                    value:
                      sqlResults.find(
                        (q) => q.name === "Sanctions Related Entities",
                      )?.count ?? 0,
                    accent: true,
                  },
                  {
                    label: "Emails Found",
                    value: ge?.emails?.count ?? 0,
                    accent: false,
                  },
                  {
                    label: "Slack Mentions",
                    value: ge?.slack?.count ?? 0,
                    accent: false,
                  },
                  {
                    label: "Confidence",
                  ...
                  <div
                    key={label}
                    className="bg-[#F7F3EE] px-6 py-5 flex flex-col gap-1"
                  >
                    <span className="text-[10px] tracking-[0.15em] text-[#9B8E82] font-outfit uppercase">
                      {label}
                    </span>
              ...
              <div className="grid grid-cols-2 gap-px bg-[#E0D8CF] border-b border-[#E0D8CF]">
                ...
                <div className="bg-[#F7F3EE] p-8">
                  <div className="flex items-center gap-2.5 mb-6">
                    <div
                      ...
                      style={{ height: "1.5px" }}
                    />
                    <span className="text-[10px] tracking-[0.2em] text-[#9B8E82] uppercase font-outfit">
                      Key Findings
                    </span>
                  </div>
                  <div className="flex flex-col gap-5">
                    {(report.key_findings || []).map((f, i) => (
                      <div key={i} className="flex gap-4 items-start">
                        <div
                          className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold flex-shrink-0 mt-0.5 border font-outfit ${rc.text} ${rc.border} ${rc.bg}`}
                        >
                          {i + 1}
                        </div>
                        <span className="text-[13px] font-light text-[#4A3E35] leading-[1.7]">
                          {f}
                        </span>
                      </div>
                    ))}
                ...
                <div className="flex flex-col gap-px bg-[#E0D8CF]">
                  {/* Sanctions types */}
                  {ge?.sanctions?.types && ge.sanctions.types.length > 0 && (
                    <div className="bg-[#F7F3EE] p-7 flex-1">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {[...new Set(ge.sanctions.types)].map((t, i) => (
                          <span
                            key={i}
                            className="px-3 py-1.5 bg-[#F5EAE0] border border-[#E8CDB8] rounded-full text-[10px] font-semibold tracking-widest text-[#C06030] font-outfit"
                          >
                            {t}
                          </span>
                        ))}
                  ...
                  >
                    <div className="flex items-center gap-2.5 mb-3">
                      ...
                      />
                      <span
                        className={`text-[10px] tracking-[0.2em] uppercase font-outfit opacity-70 ${rc.text}`}
                      >
                        Recommended Action
                      </span>
                    </div>
                    <div
                      className={`font-playfair text-2xl font-bold ${rc.text}`}
                    >
                      {report.recommended_action}
                    </div>
                    {report.investigator_notes && (
                      <p className="text-xs font-light text-[#6B5E52] mt-3 leading-relaxed">
                        {report.investigator_notes}
                      </p>
                    )}
                  </div>
                  ...
                  {report.network_analysis && (
                    <div className="bg-[#F7F3EE] p-7">
                      ...
                      <p className="text-[13px] font-light text-[#6B5E52] leading-[1.7]">
                        {report.network_analysis}
                      </p>
                    </div>
                  )}
                ...
                <div className="bg-[#F7F3EE]">
                  ...
                  <div>
                    {report.evidence.map((ev, i) => (
                      <div
                        key={i}
                        className={`grid grid-cols-[180px_1fr] gap-4 px-8 py-4 items-start border-b border-[#E0D8CF] ${i % 2 === 0 ? "bg-[#F7F3EE]" : "bg-white"}`}
                      >
                        <span
                          className={`inline-flex px-2.5 py-1 rounded-full text-[10px] font-semibold tracking-wide border w-fit font-outfit ${getSourceStyle(ev.source)}`}
                        >
                          {ev.source.toUpperCase()}
                        </span>
                        <span className="text-[13px] font-light text-[#6B5E52] leading-[1.65]">
                          {ev.finding ||
                            (ev as ReportEvidence & { detail?: string }).detail}
                        </span>
                      </div>
                    ))}
                  </div>
              ...
              <div className="bg-[#EDE7DF] border-t border-[#D8CEBF] px-8 py-4 flex justify-between items-center">
                <span className="text-[10px] tracking-widest text-[#C4B8AC] font-outfit uppercase">
                  Sentinel — Sanctions & Intelligence Screening
                </span>
                <div className="flex gap-6">
                  {[
                    ...
                    <Link
                      key={href}
                      href={href}
                      className="text-[11px] tracking-wide text-[#9B8E82] hover:text-[#D97B4F] transition-colors no-underline font-outfit"
                    >
                      {label} →
                    </Link>
                  ))}
          ...
          {!result && !loading && (
            <div className="text-center py-32">
              <div className="font-playfair text-7xl text-[#E0D8CF] mb-6">
                ⬡
              </div>
